# refer pytorch 3d unet: https://github.com/wolny/pytorch-3dunet/blob/15852eac812a41d07202d172523964b7d1e48b68/pytorch3dunet/unet3d/buildingblocks.py#L98
...
def create_conv(in_channels, out_channels, kernel_size, order, num_groups,
    ...
    for i, char in enumerate(order):
        if char == 'r':
            ...
            bias = False if ('g' in order or 'b' in order) else None
            modules.append(('conv',
                            nn.Conv3D(
                                in_channels,
                                out_channels,
...
class Encoder(nn.Layer):
    ...
    Args:
        ...
        basic_module(nn.Layer): either ResNetBlock or DoubleConv
...
class Decoder(nn.Layer):
    ...
    def _joining(encoder_features, x, concat):
        if concat:
            return paddle.concat((encoder_features, x), axis=1)
        else:
...
def create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding,
    ...
    return nn.LayerList(decoders)


class AbstractUpsampling(nn.Layer):
    """
