...
import tools.preprocess_utils.preprocessing
import matplotlib
import numpy as np
import paddle
...
from medicalseg.core.training.dataloading.dataset_loading import load_dataset, DataLoader3D, DataLoader2D, unpack_dataset
from medicalseg.models.losses.dice_loss import DC_and_CE_loss
from medicalseg.core.training.network_training.network_trainer import NetworkTrainer
...
class nnUNetTrainer(NetworkTrainer):
    ...
    def initialize_optimizer_and_scheduler(self):
        ...
        self.lr_scheduler = ReduceOnPlateau(self.initial_lr, mode='min', factor=0.2,
                                                          patience=self.lr_scheduler_patience,
                                                          verbose=True, threshold=self.lr_scheduler_eps,
                                                          threshold_mode="abs")
        self.optimizer = paddle.optimizer.Adam(self.lr_scheduler ,parameters=self.network.parameters(), weight_decay=self.weight_decay)
    ...
    def plot_network_architecture(self):
        try:
            ...
            if paddle.fluid.is_compiled_with_cuda():
                g = hl.build_graph(self.network, paddle.rand((1, self.num_input_channels, *self.patch_size)).cuda(),
                                   transforms=None)
            else:
                g = hl.build_graph(self.network, paddle.rand((1, self.num_input_channels, *self.patch_size)),
                                   transforms=None)
    ...
    def preprocess_patient(self, input_files):
        ...
        preprocessor_class = recursive_find_python_class([join(tools.preprocess_utils.__path__[0], "preprocessing")],
                                                         preprocessor_name,
                                                         current_module="tools.preprocess_utils.preprocessing")
    ...
    def predict_preprocessed_data_return_seg_and_softmax(self, data: np.ndarray, do_mirroring: bool = True,
        ...
        valid = list((SegmentationNetwork, paddle.DataParallel))
    ...
    def run_online_evaluation(self, output, target):
        with paddle.no_grad():
            ...
            # import pdb
            # pdb.set_trace()
            output_softmax=paddle.cast(output_softmax,'float32')
            ...
            for c in range(1, num_classes):
                tp_hard[:, c - 1] = sum_tensor(paddle.cast((output_seg == c),"float32")* paddle.cast((target == c),"float32"), axes=axes)
                fp_hard[:, c - 1] = sum_tensor(paddle.cast((output_seg == c),"float32") * paddle.cast((target != c),"float32"), axes=axes)
                fn_hard[:, c - 1] = sum_tensor(paddle.cast((output_seg != c),"float32") * paddle.cast((target == c),"float32"), axes=axes)
