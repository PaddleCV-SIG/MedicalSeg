...
import numpy as np
from tools.batchgenerators.augmentations.utils import pad_nd_image
from tools.utilities.random_stuff import no_op
from tools.utilities.to_paddle import to_cuda, maybe_to_paddle
...
class SegmentationNetwork(nn.Layer):
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        if all_in_gpu:
            ...
            aggregated_results = paddle.zeros([self.num_classes] + list(data.shape[1:]), dtype=paddle.float16)

            if verbose: print("moving data to GPU")
            data = paddle.to_tensor(data).cuda(self.get_device(), non_blocking=True)

            if verbose: print("initializing result_numsamples (on GPU)")
            aggregated_nb_of_predictions = paddle.zeros([self.num_classes] + list(data.shape[1:]), dtype=paddle.float16)
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x, mirror_axes,
                                           do_mirroring,
                                           mult):
        ...
        x = maybe_to_paddle(x)
        result_paddle = paddle.zeros([1, self.num_classes] + list(x.shape[2:]),
                                   dtype=paddle.float32)



        if mult is not None:
            mult = maybe_to_paddle(mult)
    ...
    def _internal_maybe_mirror_and_pred_2D(self, x, mirror_axes,
        ...
        x = maybe_to_paddle(x)
        result_paddle = paddle.zeros([x.shape[0], self.num_classes] + list(x.shape[2:]), dtype=paddle.float32)
