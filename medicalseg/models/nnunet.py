...
class NNUNet(nn.Layer):
    ...
    def __init__(self, 
        ...
        self.network = Generic_UNet(input_channels=self.num_input_channels, 
    ...
    def init_weight(self):
        if self.pretrained is not None:
            ...
            for sublayer in self.sublayers():
                if isinstance(sublayer, (nn.Conv2D, nn.Conv3D, nn.Conv2DTranspose, nn.Conv3DTranspose)):
    ...
    def load_and_process_plan_file(self, plan_path):
        ...
        if self.stage is None:
            assert len(list(plans['plans_per_stage'].keys())) == 1, "If self.stage is None then there can be only one stage in the plans file but got {}. Please specify which stage of the cascade must be trained.".format(len(list(plans['plans_per_stage'].keys())))
...
class Generic_UNet(nn.Layer):
    ...
    def __init__(self, 
        ...
        if conv_op == nn.Conv2D:
            ...
            raise ValueError("Unknown convolution dimensionality, conv op: {}.".format(str(conv_op)))
        ...
        for ds in range(len(self.conv_blocks_localization)):
            self.seg_heads.append(conv_op(self.conv_blocks_localization[ds][-1].output_channels, num_classes,
