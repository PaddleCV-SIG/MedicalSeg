...
def results_loop(in_queues: List[Queue], out_queue: thrQueue, abort_event: Event, pin_memory: bool,
    ...
    do_pin_memory = paddle is not None and pin_memory and gpu is not None and paddle.is_compiled_with_cuda()
...
class MultiThreadedAugmenter(object):
    ...
    def __init__(self, data_loader, transform, num_processes, num_cached_per_queue=2, seeds=None, pin_memory=False,
        ...
        print("num_process:{}".format(num_processes))
        num_processes=1
    ...
    def _start(self):
        if not self.was_initialized:
            ...
            if paddle is not None and paddle.fluid.is_compiled_with_cuda():
                gpu = paddle.device.cuda.get_device_name()
            else:
                gpu = None
