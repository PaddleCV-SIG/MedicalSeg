...
class SegmentationNetwork:
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        if all_in_gpu:
            ...
            aggregated_results = paddle.zeros([self.num_classes] + list(data.shape[1:]), dtype=paddle.half)

            if verbose: print("moving data to GPU")
            data = paddle.to_tensor(data)

            if verbose: print("initializing result_numsamples (on GPU)")
            aggregated_nb_of_predictions = paddle.zeros([self.num_classes] + list(data.shape[1:]), dtype=paddle.half)
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, paddle.Tensor], mirror_axes: tuple,
        ...
        x = maybe_to_torch(x)
        result_torch = paddle.zeros([1, self.num_classes] + list(x.shape[2:]),
                                   dtype='float32')
        ...
        for m in range(mirror_idx):
            ...
            if m == 1 and (2 in mirror_axes):
                ...
                result_torch += 1 / num_results * paddle.flip(pred, (4,))

            if m == 2 and (1 in mirror_axes):
                pred = self.inference_apply_nonlin(self(paddle.flip(x, (3, ))))
                result_torch += 1 / num_results * paddle.flip(pred, (3,))

            if m == 3 and (2 in mirror_axes) and (1 in mirror_axes):
                pred = self.inference_apply_nonlin(self(paddle.flip(x, (4, 3))))
                result_torch += 1 / num_results * paddle.flip(pred, (4, 3))

            if m == 4 and (0 in mirror_axes):
                pred = self.inference_apply_nonlin(self(paddle.flip(x, (2, ))))
                result_torch += 1 / num_results * paddle.flip(pred, (2,))

            if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):
                pred = self.inference_apply_nonlin(self(paddle.flip(x, (4, 2))))
                result_torch += 1 / num_results * paddle.flip(pred, (4, 2))

            if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):
                pred = self.inference_apply_nonlin(self(paddle.flip(x, (3, 2))))
                result_torch += 1 / num_results * paddle.flip(pred, (3, 2))

            if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):
                pred = self.inference_apply_nonlin(self(paddle.flip(x, (4, 3, 2))))
                result_torch += 1 / num_results * paddle.flip(pred, (4, 3, 2))
    ...
    def _internal_maybe_mirror_and_pred_2D(self, x: Union[np.ndarray, paddle.Tensor], mirror_axes: tuple,
                                           do_mirroring: bool = True,
                                           mult: np.ndarray or paddle.Tensor = None) -> paddle.Tensor:
        assert len(x.shape) == 4, 'x must be (b, c, x, y)'

        x = maybe_to_torch(x)
        result_torch = paddle.zeros([x.shape[0], self.num_classes] + list(x.shape[2:]), dtype='float32')
...
class Generic_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
                 ...
                 final_nonlin=nn.Softmax(axis=1), weightInitializer=1e-2, pool_op_kernel_sizes=None,
        ...
        for ds in range(len(self.conv_blocks_localization)):
            self.seg_outputs.append(conv_op(self.conv_blocks_localization[ds][-1].output_channels, num_classes,
                                            1, 1, 0, 1, 1, bias_attr=seg_output_use_bias))
        ...
        self.conv_blocks_localization = nn.LayerList(self.conv_blocks_localization)
        self.conv_blocks_context = nn.LayerList(self.conv_blocks_context)
        self.td = nn.LayerList(self.td)
        self.tu = nn.LayerList(self.tu)
        self.seg_outputs = nn.LayerList(self.seg_outputs)
        if self.upscale_logits:
            self.upscale_logits_ops = nn.LayerList(
                self.upscale_logits_ops)  # lambda x:x is not a Module so we need to distinguish here
        self.apply(self.__weight_init)
    ...
    def __weight_init(self, module):
        if isinstance(module, nn.Conv3D) or isinstance(module, nn.Conv2D) or isinstance(module, nn.Conv2DTranspose) or isinstance(module, nn.Conv3DTranspose):
            nn.initializer.KaimingNormal()(module.weight)
            if module.bias is not None:
                nn.initializer.Constant(0)(module.bias)
    ...
    def forward(self, x):
        ...
        if self._deep_supervision and self.do_ds and self.training:
