...
def parse_args():
    ...
    parser.add_argument('--ensemble_folds', '--ensemble_folds', nargs='+', required=True, help="The dirs of softmax results.")
    parser.add_argument('--output_folder', '--output_folder', required=True, default='output/ensemble', help="The dirs of softmax results.")
    parser.add_argument('--num_threads', '--num_threads', type=int, default=2, help='The number of threads.')
    parser.add_argument("--gt_dir", "--gt_dir", required=False, default=None, type=str, help='grount truth dir')
    parser.add_argument("--plan_path", "--plan_path", required=False, default=None, type=str, help='If ensemble val folder, plan_path must be supplied.')
    parser.add_argument("--folds", "--folds", default=5, type=int, required=False)
    parser.add_argument('--postprocessing_json_path', "--postprocessing_json_path", required=False, default=None,
                        type=str, help='the path to postprocessing json.')
    parser.add_argument('--save_npz', action="store_true", required=False, help="stores npz and pkl")
...
def merge_files(files, properties_files, out_file, override=False, store_npz=False):
...
def merge(folders, output_folder, threads, override=True, postprocessing_file=None, store_npz=False):
    ...
    if postprocessing_file is not None:
        ...
        if 'min_valid_object_sizes' in pp_info.keys():
            min_valid_object_sizes = ast.literal_eval(pp_info['min_valid_object_sizes'])
        else:
            min_valid_object_sizes = None
        for_which_classes, min_valid_obj_size = pp_info['for_which_classes'], min_valid_object_sizes
        print('Postprocessing...')
        p = Pool(threads)
        nii_files = [file for file in os.listdir(output_folder) if file.endswith('.nii.gz')]
...
def ensemble_val(val_folder1, val_folder2, output_folder, plan_path, validation_folder, folds, gt_dir, threads=4, override=True):
    print("\nEnsembling folders: \n", val_folder1, "\n", val_folder2)
    ...
    for f in range(folds):
        validation_folder_net1 = os.path.join(val_folder1, "fold_%d" % f, validation_folder)
        validation_folder_net2 = os.path.join(val_folder2, "fold_%d" % f, validation_folder)
        ...
        if not os.path.isfile(os.path.join(validation_folder_net1, 'summary.json')):
        ...
        if not os.path.isfile(os.path.join(validation_folder_net2, 'summary.json')):
            raise AssertionError("Validation directory missing: %s. Please rerun validation with `python nnunet_tools/nnunet_fold_val.py --config {config_path} --model_path {model_path} --precision fp16 --save_dir {save_dir} --val_save_folder {val predicted save dir}`" % validation_folder_net2)

        patient_identifiers1_npz = [file[:-4] for file in os.listdir(validation_folder_net1) if file.endswith('npz')]
        patient_identifiers1_npz.sort()
        patient_identifiers2_npz = [file[:-4] for file in os.listdir(validation_folder_net2) if file.endswith('npz')]
        patient_identifiers2_npz.sort()

        patient_identifiers1_nii = [file for file in os.listdir(validation_folder_net1) if file.endswith('nii.gz')]
        patient_identifiers1_nii.sort()
        patient_identifiers1_nii = [i[:-7] for i in patient_identifiers1_nii if not i.endswith("noPostProcess.nii.gz") and not i.endswith('_postprocessed.nii.gz')]

        patient_identifiers2_nii = [file for file in os.listdir(validation_folder_net2) if file.endswith('nii.gz')]
        patient_identifiers2_nii.sort()
        patient_identifiers2_nii = [i[:-7] for i in patient_identifiers2_nii if not i.endswith("noPostProcess.nii.gz") and not i.endswith('_postprocessed.nii.gz')]

        if not all([i in patient_identifiers1_npz for i in patient_identifiers1_nii]):
            raise AssertionError("Missing npz files in folder %s. Please run the validation for all models and folds with the '--save_npz' flag." % (validation_folder_net1))
        if not all([i in patient_identifiers2_npz for i in patient_identifiers2_nii]):
            raise AssertionError("Missing npz files in folder %s. Please run the validation for all models and folds with the '--save_npz' flag." % (validation_folder_net2))

        assert all([i == j for i, j in zip(patient_identifiers1_npz, patient_identifiers2_npz)]), "The number of npz file in {} and {} are not matched.".format(validation_folder_net1, validation_folder_net2)

        os.makedirs(output_folder, exist_ok=True)

        for p in patient_identifiers1_npz:
            files.append([os.path.join(validation_folder_net1, p + '.npz'), os.path.join(validation_folder_net2, p + '.npz')])
    ...
    p = Pool(threads)
    p.starmap(merge_files, zip(files, property_files, out_files, [override] * len(out_files), [False] * len(out_files)))
    p.close()
    p.join()
    ...
    if not os.path.isfile(os.path.join(output_folder, "summary.json")) and len(out_files) > 0:
        aggregate_scores(tuple(zip(out_files, gt_segmentations)), labels=plans['all_classes'],
                     json_output_file=os.path.join(output_folder, "summary.json"), num_threads=threads)
    print("runing postprocessing...")
    determine_postprocessing(output_folder_base, folder_with_gt_segs, "ensembled_raw", "temp",
                                "ensembled_postprocessed", threads, dice_threshold=0)
    print("postprocessing done.")
...
def main(args):
    ...
    if args.gt_dir is None:
        merge(args.ensemble_folds, args.output_folder, threads=args.num_threads, postprocessing_file=args.postprocessing_json_path, store_npz=args.save_npz)
    else:
        assert args.plan_path is not None, "If ensemble val folders, please set plan_path."
        assert len(args.ensemble_folds) >= 2, "The number of ensemble folders must greater than 2."

        for folder1, folder2 in combinations(args.ensemble_folds, 2):
            ensemble_name = "ensemble_" + folder1.split('/')[-1] + "__" + folder2.split('/')[-1]
            output_folder_base = os.path.join(args.output_folder, "ensembles", ensemble_name)
            os.makedirs(output_folder_base, exist_ok=True)

            print("ensembling fold: ", folder1, folder2)
            ensemble_val(folder1, folder2, output_folder_base, args.plan_path, "validation_raw", args.folds, args.gt_dir, threads=args.num_threads, override=True)
