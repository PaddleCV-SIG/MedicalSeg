...
def downsample_seg_for_ds_transform3(seg, ds_scales=((1, 1, 1), (0.5, 0.5, 0.5), (0.25, 0.25, 0.25)), classes=None):
    output = []
    one_hot = paddle.to_tensor(convert_seg_image_to_one_hot_encoding_batched(seg, classes))
...
def augment_mirroring(sample_data, sample_seg=None, axes=(0, 1, 2)):
    if (len(sample_data.shape) != 3) and (len(sample_data.shape) != 4):
        raise Exception(  "Invalid dimension for sample_data and sample_seg. sample_data and sample_seg should be either "
    ...
    for sample_id in range(data.shape[0]):
        ...
        if modified_coords:
            ...
            if random_crop:
                ...
                d, s = random_crop(data[sample_id:sample_id + 1], s, patch_size, margin)
            else:
