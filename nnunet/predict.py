...
def parse_args():
    ...
    parser.add_argument('--output_folder', "--output_folder", required=True, help="folder for saving predictions")
    parser.add_argument('--model_type', '--model_type', required=True, type=str,
    ...
    parser.add_argument('--model_paths', '--model_paths', nargs='+', required=True, help="The multi model paths.")
    parser.add_argument('--postprocessing_json_path', "--postprocessing_json_path", required=True, default=None,
    ...
    parser.add_argument('--lowres_segmentations', '--lowres_segmentations', required=False, default=None,
    ...
    parser.add_argument('--save_npz', '--save_npz', required=False, action='store_true',
    ...
    parser.add_argument("--num_threads_preprocessing", "--num_threads_preprocessing", required=False, default=6,
                        type=int, help=
                        "Determines many background processes will be used for data preprocessing. Reduce this if you "
                        "run into out of memory (RAM) problems. Default: 6")

    parser.add_argument("--num_threads_nifti_save", "--num_threads_nifti_save", required=False, default=2, type=int,
                        help=
                        "Determines many background processes will be used for segmentation export. Reduce this if you "
                        "run into out of memory (RAM) problems. Default: 2")

    parser.add_argument("--mode", "--mode", type=str, default="normal", required=False, help="Hands off!")
    parser.add_argument("--step_size", "--step_size", type=float, default=0.5, required=False, help="don't touch")
    parser.add_argument("--overwrite_existing", "--overwrite_existing", required=False, default=False,
                        action="store_true",
                        help="Set this flag if the target folder contains predictions that you would like to overwrite")
    parser.add_argument("--disable_postprocessing", "--disable_postprocessing", required=False, default=False,
                        action="store_true",
    ...
    parser.add_argument('--precision', default='fp16', required=False, help="whether use mixed precision prediction.")
    return parser.parse_args()


def check_input_folder_and_return_caseIDs(input_folder, expected_num_modalities):
    print("This model expects {} input modalities for each image.".format(expected_num_modalities))
    files = [file for file in os.listdir(input_folder) if
             os.path.isfile(os.path.join(input_folder, file)) and file.endswith(".nii.gz")]
    files.sort()
    ...
    print("Found {} unique case ids, here are some examples: ".format(len(maybe_case_ids)),
          np.random.choice(maybe_case_ids, min(len(maybe_case_ids), 10)))
    print("If they don't look right, make sure to double check your filenames. They must end with _0000.nii.gz etc")

    if len(remaining) > 0:
        print("found {} unexpected remaining files in the folder. Here are some examples:".format(len(remaining)),
...
def predict_from_folder(predictor,
    ...
    assert os.path.exists(plan_path), "plan file path: {} not exists.".format(plan_path)
    ...
    all_files = [file for file in os.listdir(input_folder) if
                 os.path.isfile(os.path.join(input_folder, file)) and file.endswith(".nii.gz")]
    all_files.sort()
    ...
    if lowres_segmentations is not None:
        assert os.path.isdir(
            lowres_segmentations), "if lowres_segmentations is not None then it must point to a directory"
    ...
    if mode == "normal":
        return predict_cases(predictor=predictor,
...
def main(args):
    assert args.model_type in ['2d', '3d', 'cascade_lowres',
                               'cascade_fullres'], "model only support ['2d', '3d', 'cascade_lowres', 'cascade_fullres'], but got {}.".format(
        args.model_type)
    ...
    if args.lowres_segmentations is not None:
        ...
        print("Cascade lowres segmentation result dir: ", args.lowres_segmentations)

    predict_from_folder(predictor=predictor,
