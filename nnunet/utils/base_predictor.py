...
class BasePredictor:
                ...
                if self.threeD:
                    ...
                    if use_sliding_window:
                        res = self._internal_predict_3D_2Dconv_tiled(
                            ...
                            step_size, regions_class_order, use_gaussian,
                            pad_border_mode, pad_kwargs, False)
                    else:
    ...
    def _compute_steps_for_sliding_window(patch_size: Tuple[int, ...],
        ...
        num_steps = [
            int(np.ceil((i - k) / j)) + 1 for i, j, k in
    ...
    def _internal_predict_3D_3Dconv_tiled(
        ...
        slicer = tuple([
            slice(0, aggregated_results.shape[i]) for i in range(
                len(aggregated_results.shape) - (len(slicer) - 1))
        ] + slicer[1:])
