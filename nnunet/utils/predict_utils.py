...
def preprocess_multithreaded(predictor, list_of_lists, output_files, num_processes=2, segs_from_prev_stage=None):
    ...
    for i in range(num_processes):
        pr = Process(target=preprocess_save_to_queue, args=(predictor.preprocess_patient, q,
                                                            list_of_lists[i::num_processes],
                                                            output_files[i::num_processes],
                                                            segs_from_prev_stage[i::num_processes],
                                                            classes, predictor.plans['transpose_forward']))
        pr.start()
        processes.append(pr)
...
def predict_cases(predictor,
    ...
    assert len(list_of_lists) == len(
        output_filenames), "The number of input files and output files is not same. Please check your input folder."
    if segs_from_prev_stage is not None:
        assert len(segs_from_prev_stage) == len(
            output_filenames), "The cascade lowres predict results is not the same with the number of output files."
    ...
    if not overwrite_existing:
        ...
        not_done_idx = [i for i, j in enumerate(cleaned_output_files) if
                        (not os.path.isfile(j)) or (save_npz and not os.path.isfile(j[:-7] + '.npz'))]
    ...
    if segmentation_export_kwargs is None:
        if 'segmentation_export_params' in predictor.plans.keys():
            force_separate_z = predictor.plans['segmentation_export_params']['force_separate_z']
            interpolation_order = predictor.plans['segmentation_export_params']['interpolation_order']
            interpolation_order_z = predictor.plans['segmentation_export_params']['interpolation_order_z']
        else:
    ...
    preprocessing = preprocess_multithreaded(predictor, list_of_lists, cleaned_output_files, num_threads_preprocessing,
